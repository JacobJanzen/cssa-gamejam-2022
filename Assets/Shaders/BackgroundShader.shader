Shader "Custom/BackgroundShader"
{
		...
		SubShader
	{
		...
		Pass
		{
			...
			float4 starBrightness(float starNum) {
			...
			float4 frag(float4 sp:VPOS) : SV_Target {
				...
				sunValue = easeOutQuart(sunValue);
				sunValue *= max(0, 1 - easeOutQuart(altitudeRatio) * 1.5);
				...
				moonValue *= 1 - max(0, 1 - easeOutQuart(altitudeRatio) * 1.5);
				...
				float moonPatternAlpha = min(1, max(0, 1 - max(35, moonCenterDist) / moonRadius * 3)) * easeOutQuart(altitudeRatio);
